/**
 * \file
 * test/protocol/test_vcblockchain_protocol_encode_resp_artifact_first_txn_id_get.cpp
 *
 * Unit tests for encoding the artifact first txn id get response.
 *
 ...
 */
TEST(test_vcblockchain_protocol_encode_resp_artifact_first_txn_id_get, parameters)
{
...
TEST(test_vcblockchain_protocol_encode_resp_artifact_first_txn_id_get, happy_path)
{
    const uint32_t EXPECTED_OFFSET = 26;
    const uint32_t EXPECTED_STATUS = 11;
    ...
    EXPECT_EQ(htonl(PROTOCOL_REQ_ID_ARTIFACT_FIRST_TXN_BY_ID_GET), uarr[0]);
