/**
 * \file
 * test/protocol/test_vcblockchain_protocol_decode_req_connection_close.cpp
 *
 * Unit tests for decoding the connection close request.
 *
 ...
 */
TEST(test_vcblockchain_protocol_decode_req_connection_close, parameter_check)
{
 ...
 */
TEST(test_vcblockchain_protocol_decode_req_connection_close, payload_size)
{
    const uint8_t EXPECTED_PAYLOAD[4] = { 0x00, 0x01, 0x02, 0x03 };
    size_t EXPECTED_PAYLOAD_SIZE = sizeof(EXPECTED_PAYLOAD);
    protocol_req_connection_close req;
 ...
 */
TEST(test_vcblockchain_protocol_decode_req_connection_close, happy_path)
{
    ...
    EXPECT_EQ(PROTOCOL_REQ_ID_CLOSE, req.request_id);
