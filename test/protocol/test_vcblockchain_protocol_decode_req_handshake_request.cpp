...
TEST_SUITE(test_vcblockchain_protocol_decode_req_handshake_request);
...
TEST(basics)
{
    ...
    TEST_EXPECT(PROTOCOL_REQ_ID_HANDSHAKE_INITIATE == req.request_id);

    /* the offset should match. */
    TEST_EXPECT(EXPECTED_OFFSET == req.offset);

    /* the protocol version should match what we expect. */
    TEST_EXPECT(PROTOCOL_VERSION_0_1_DEMO == req.protocol_version);

    /* the crypto suite should match what we expect. */
    TEST_EXPECT(VCCRYPT_SUITE_VELO_V1 == req.crypto_suite);

    /* the client id should match. */
    TEST_EXPECT(0 == memcmp(&client_id, &req.client_id, sizeof(client_id)));

    /* the key nonce should match. */
    TEST_EXPECT(client_key_nonce.size == req.client_key_nonce.size);
    TEST_EXPECT(
        0
            == memcmp(
    ...
    TEST_EXPECT(client_challenge_nonce.size == req.client_challenge_nonce.size);
    TEST_EXPECT(
        0
            == memcmp(
                    client_challenge_nonce.data,
                    req.client_challenge_nonce.data,
