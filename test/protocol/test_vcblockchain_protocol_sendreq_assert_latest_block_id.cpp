/**
 * \file
 * test/protocol/test_vcblockchain_protocol_sendreq_assert_latest_block_id.cpp
 *
 * Unit tests for writing an assert latest block request to a server socket.
 *
 ...
 * Test the happy path.
 */
TEST(test_vcblockchain_protocol_assert_latest_block_id, happy_path)
{
    ssock sock;
    ...
    const vpr_uuid EXPECTED_LATEST_BLOCK_ID = { .data = {
        0xcc, 0xc1, 0xa4, 0x40, 0xf5, 0x09, 0x47, 0xe8,
        0x92, 0x71, 0xe6, 0xcc, 0xc5, 0x6e, 0xff, 0xf4
    } };
    ...
    EXPECT_EQ(PROTOCOL_REQ_ID_ASSERT_LATEST_BLOCK_ID, req.request_id);
    EXPECT_EQ(EXPECTED_OFFSET, req.offset);
    EXPECT_EQ(0, memcmp(&EXPECTED_LATEST_BLOCK_ID, &req.latest_block_id, 16));
