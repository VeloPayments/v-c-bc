/**
 * \file
 * test/protocol/test_vcblockchain_protocol_decode_resp_latest_block_id_get.cpp
 *
 * Unit tests for decoding the latest block id get response.
 *
 ...
 */
TEST(test_vcblockchain_protocol_decode_resp_latest_block_id_get, parameters)
{
...
/**
 * This method should check the payload size to make sure it is what it expects.
 */
TEST(test_vcblockchain_protocol_decode_resp_latest_block_id_get, payload_size)
{
    const uint8_t EXPECTED_PAYLOAD[4] = { 0x00, 0x01, 0x02, 0x03 };
    size_t EXPECTED_PAYLOAD_SIZE = sizeof(EXPECTED_PAYLOAD);
    protocol_resp_latest_block_id_get resp;
 ...
 */
TEST(test_vcblockchain_protocol_decode_resp_latest_block_id_get, happy_path)
{
    ...
    EXPECT_EQ(PROTOCOL_REQ_ID_LATEST_BLOCK_ID_GET, resp.request_id);
