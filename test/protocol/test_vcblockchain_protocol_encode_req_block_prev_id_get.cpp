/**
 * \file
 * test/protocol/test_vcblockchain_protocol_encode_req_block_prev_id_get.cpp
 *
 * Unit tests for encoding the block prev id get request.
 *
 ...
 */
TEST(test_vcblockchain_protocol_encode_req_block_prev_id_get, parameter_check)
{
 ...
 */
TEST(test_vcblockchain_protocol_encode_req_block_prev_id_get, happy_path)
{
    const uint32_t EXPECTED_OFFSET = 97;
    allocator_options_t alloc_opts;
    vccrypt_buffer_t buffer;
    ...
    EXPECT_EQ(htonl(PROTOCOL_REQ_ID_BLOCK_ID_GET_PREV), u32arr[0]);
