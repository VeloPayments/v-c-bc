/**
 * \file
 * test/protocol/test_vcblockchain_protocol_encode_req_artifact_first_txn_id_get.cpp
 *
 * Unit tests for encoding the artifact first txn id get request.
 *
 ...
 */
TEST(
    test_vcblockchain_protocol_encode_req_artifact_first_txn_id_get,
    parameter_check)
{
    ...
    EXPECT_EQ(
        ...
        vcblockchain_protocol_encode_req_artifact_first_txn_id_get(
            nullptr, &alloc_opts, EXPECTED_OFFSET, &EXPECTED_ARTIFACT_ID));
    EXPECT_EQ(
        VCBLOCKCHAIN_ERROR_INVALID_ARG,
        vcblockchain_protocol_encode_req_artifact_first_txn_id_get(
            &buffer, nullptr, EXPECTED_OFFSET, &EXPECTED_ARTIFACT_ID));
    EXPECT_EQ(
        VCBLOCKCHAIN_ERROR_INVALID_ARG,
        vcblockchain_protocol_encode_req_artifact_first_txn_id_get(
 ...
 */
TEST(
    test_vcblockchain_protocol_encode_req_artifact_first_txn_id_get, happy_path)
{
    const uint32_t EXPECTED_OFFSET = 97;
    allocator_options_t alloc_opts;
    vccrypt_buffer_t buffer;
    ...
    EXPECT_EQ(htonl(PROTOCOL_REQ_ID_ARTIFACT_FIRST_TXN_BY_ID_GET), u32arr[0]);
    ...
    EXPECT_EQ(0, memcmp(barr, &EXPECTED_ARTIFACT_ID, 16));
