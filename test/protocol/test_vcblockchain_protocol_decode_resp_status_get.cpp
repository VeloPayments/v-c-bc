/**
 * \file
 * test/protocol/test_vcblockchain_protocol_decode_resp_status_get.cpp
 *
 * Unit tests for decoding the status get response.
 *
 ...
 */
TEST(test_vcblockchain_protocol_decode_resp_status_get, payload_size)
{
    const uint8_t EXPECTED_PAYLOAD[4] = { 0x00, 0x01, 0x02, 0x03 };
    size_t EXPECTED_PAYLOAD_SIZE = sizeof(EXPECTED_PAYLOAD);
    protocol_resp_status_get resp;
 ...
 */
TEST(test_vcblockchain_protocol_decode_resp_status_get, happy_path)
{
    ...
    ASSERT_EQ(
        ...
        vcblockchain_protocol_encode_resp_status_get(
    ...
    EXPECT_EQ(PROTOCOL_REQ_ID_STATUS_GET, resp.request_id);
