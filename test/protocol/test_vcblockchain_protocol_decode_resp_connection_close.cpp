/**
 * \file
 * test/protocol/test_vcblockchain_protocol_decode_resp_connection_close.cpp
 *
 * Unit tests for decoding the connection close response.
 *
 ...
 */
TEST(test_vcblockchain_protocol_decode_resp_connection_close, parameters)
{
 ...
 */
TEST(test_vcblockchain_protocol_decode_resp_connection_close, payload_size)
{
    const uint8_t EXPECTED_PAYLOAD[4] = { 0x00, 0x01, 0x02, 0x03 };
    size_t EXPECTED_PAYLOAD_SIZE = sizeof(EXPECTED_PAYLOAD);
    protocol_resp_connection_close resp;
 ...
 */
TEST(test_vcblockchain_protocol_decode_resp_connection_close, happy_path)
{
    ...
    EXPECT_EQ(PROTOCOL_REQ_ID_CLOSE, resp.request_id);
