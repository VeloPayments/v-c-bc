...
TEST_SUITE(test_vcblockchain_protocol_decode_resp_block_get);
...
TEST(happy_path)
{
    ...
    TEST_EXPECT(PROTOCOL_REQ_ID_BLOCK_BY_ID_GET == resp.request_id);
    ...
    TEST_EXPECT(0 == memcmp(&resp.prev_block_id, &EXPECTED_PREV_BLOCK_ID, 16));
    ...
    TEST_EXPECT(0 == memcmp(&resp.first_txn_id, &EXPECTED_FIRST_TXN_ID, 16));
    /* the block height is set correctly. */
    TEST_EXPECT(resp.block_height == EXPECTED_BLOCK_HEIGHT);
    /* the serialized block size is set correctly. */
    TEST_EXPECT(resp.block_size == EXPECTED_SER_BLOCK_CERT_SIZE);
    /* the block cert is set correctly. */
    TEST_ASSERT(EXPECTED_BLOCK_CERT_SIZE == resp.block_cert.size);
    TEST_EXPECT(
        0
            == memcmp(
