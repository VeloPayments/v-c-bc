 ...
 */
TEST(
    test_vcblockchain_protocol_decode_req_assert_latest_block_id,
    parameter_check)
{
 ...
 */
TEST(test_vcblockchain_protocol_decode_req_assert_latest_block_id, payload_size)
{
    const uint8_t EXPECTED_PAYLOAD[4] = { 0x00, 0x01, 0x02, 0x03 };
    size_t EXPECTED_PAYLOAD_SIZE = sizeof(EXPECTED_PAYLOAD);
    protocol_req_assert_latest_block_id req;
 ...
 */
TEST(test_vcblockchain_protocol_decode_req_assert_latest_block_id, happy_path)
{
    ...
    EXPECT_EQ(0, memcmp(&latest_block_id, &req.latest_block_id, 16));
