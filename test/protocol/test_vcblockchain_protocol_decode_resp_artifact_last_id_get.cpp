...
TEST_SUITE(test_vcblockchain_protocol_decode_resp_artifact_last_txn_id_get);
...
TEST(happy_path)
{
    ...
    TEST_EXPECT(PROTOCOL_REQ_ID_ARTIFACT_LAST_TXN_BY_ID_GET == resp.request_id);
    ...
    TEST_EXPECT(0 == memcmp(&resp.last_txn_id, &EXPECTED_LAST_TXN_ID, 16));
