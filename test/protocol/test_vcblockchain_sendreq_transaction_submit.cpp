/**
 * \file
 * test/protocol/test_vcblockchain_protocol_sendreq_transaction_submit.cpp
 *
 * Unit tests for writing the transaction submit request to a server socket.
 *
...
TEST_SUITE(test_vcblockchain_protocol_transaction_submit);
...
TEST(happy_path)
{
    ...
    const vpr_uuid EXPECTED_TXN_ID = { .data = {
        0xbc, 0xd5, 0xc2, 0x5e, 0x46, 0x9b, 0x43, 0xa9,
        0x97, 0xda, 0x72, 0xba, 0x35, 0xb8, 0xf5, 0x71 } };
    const vpr_uuid EXPECTED_ARTIFACT_ID = { .data = {
        0xf5, 0xef, 0x41, 0x2b, 0x1b, 0xf8, 0x46, 0x22,
        0x82, 0x3e, 0x24, 0x9c, 0xee, 0x4b, 0x91, 0x80 } };
    const uint8_t EXPECTED_CERT[3] = { 0x01, 0x02, 0x03 };
    ...
    TEST_ASSERT(
        VCCRYPT_STATUS_SUCCESS
            == vcblockchain_protocol_sendreq_transaction_submit(
                    ...
                    &EXPECTED_TXN_ID, &EXPECTED_ARTIFACT_ID, EXPECTED_CERT,
                    EXPECTED_CERT_SIZE));
    ...
    TEST_EXPECT(PROTOCOL_REQ_ID_TRANSACTION_SUBMIT == req.request_id);
    ...
    TEST_ASSERT(EXPECTED_CERT_SIZE == req.cert.size);
    TEST_EXPECT(0 == memcmp(req.cert.data, EXPECTED_CERT, EXPECTED_CERT_SIZE));
