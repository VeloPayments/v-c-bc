/**
 * \file
 * test/protocol/test_vcblockchain_protocol_decode_req_txn_next_id_get.cpp
 *
 * Unit tests for decoding the txn next id get request.
 *
 ...
 */
TEST(test_vcblockchain_protocol_decode_req_txn_next_id_get, parameter_check)
{
 ...
 */
TEST(test_vcblockchain_protocol_decode_req_txn_next_id_get, payload_size)
{
    const uint8_t EXPECTED_PAYLOAD[4] = { 0x00, 0x01, 0x02, 0x03 };
    size_t EXPECTED_PAYLOAD_SIZE = sizeof(EXPECTED_PAYLOAD);
    protocol_req_txn_next_id_get req;
 ...
 */
TEST(test_vcblockchain_protocol_decode_req_txn_next_id_get, happy_path)
{
    ...
    EXPECT_EQ(PROTOCOL_REQ_ID_TRANSACTION_ID_GET_NEXT, req.request_id);
