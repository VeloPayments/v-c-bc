/**
 * \file
 * test/protocol/test_vcblockchain_protocol_encode_resp_txn_prev_id_get.cpp
 *
 * Unit tests for encoding the txn prev id get response.
 *
 ...
 */
TEST(test_vcblockchain_protocol_encode_resp_txn_prev_id_get, parameters)
{
...
TEST(test_vcblockchain_protocol_encode_resp_txn_prev_id_get, happy_path)
{
    const uint32_t EXPECTED_OFFSET = 26;
    const uint32_t EXPECTED_STATUS = 11;
    ...
    EXPECT_EQ(htonl(PROTOCOL_REQ_ID_TRANSACTION_ID_GET_PREV), uarr[0]);
    ...
    EXPECT_EQ(0, memcmp(barr, &EXPECTED_TXN_ID, sizeof(EXPECTED_TXN_ID)));
