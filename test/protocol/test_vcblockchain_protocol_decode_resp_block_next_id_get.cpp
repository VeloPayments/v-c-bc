 ...
 */
TEST(test_vcblockchain_protocol_decode_resp_block_next_id_get, parameters)
{
 ...
 */
TEST(test_vcblockchain_protocol_decode_resp_block_next_id_get, payload_size)
{
    const uint8_t EXPECTED_PAYLOAD[4] = { 0x00, 0x01, 0x02, 0x03 };
    size_t EXPECTED_PAYLOAD_SIZE = sizeof(EXPECTED_PAYLOAD);
    protocol_resp_block_next_id_get resp;
 ...
 */
TEST(test_vcblockchain_protocol_decode_resp_block_next_id_get, happy_path)
{
    ...
    EXPECT_EQ(PROTOCOL_REQ_ID_BLOCK_ID_GET_NEXT, resp.request_id);
