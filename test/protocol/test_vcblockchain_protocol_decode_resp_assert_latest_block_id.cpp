/**
 * \file
 * test/protocol/test_vcblockchain_protocol_decode_resp_assert_latest_block_id.cpp
 *
 * Unit tests for decoding a latest block id assertion response.
 *
 ...
 */
TEST(test_vcblockchain_protocol_decode_resp_assert_latest_block_id, parameters)
{
    ...
    EXPECT_EQ(
        ...
        vcblockchain_protocol_decode_resp_assert_latest_block_id(
 ...
 */
TEST(
    test_vcblockchain_protocol_decode_resp_assert_latest_block_id, payload_size)
{
    const uint8_t EXPECTED_PAYLOAD[4] = { 0x00, 0x01, 0x02, 0x03 };
    size_t EXPECTED_PAYLOAD_SIZE = sizeof(EXPECTED_PAYLOAD);
    protocol_resp_assert_latest_block_id resp;
 ...
 */
TEST(test_vcblockchain_protocol_decode_resp_assert_latest_block_id, happy_path)
{
    ...
    EXPECT_EQ(PROTOCOL_REQ_ID_ASSERT_LATEST_BLOCK_ID, resp.request_id);
