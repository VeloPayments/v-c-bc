...
TEST_SUITE(test_vcblockchain_protocol_recvresp);
    ...
    TEST_EXPECT(
        VCBLOCKCHAIN_ERROR_INVALID_ARG
            == vcblockchain_protocol_recvresp(
                    nullptr, alloc, &suite, &server_iv, &shared_secret,
                    &response));
    TEST_EXPECT(
        VCBLOCKCHAIN_ERROR_INVALID_ARG
            == vcblockchain_protocol_recvresp(
                    sock, nullptr, &suite, &server_iv, &shared_secret,
                    &response));
    TEST_EXPECT(
        VCBLOCKCHAIN_ERROR_INVALID_ARG
            == vcblockchain_protocol_recvresp(
                    sock, alloc, nullptr, &server_iv, &shared_secret,
                    &response));
    TEST_EXPECT(
        VCBLOCKCHAIN_ERROR_INVALID_ARG
            == vcblockchain_protocol_recvresp(
...
TEST(happy_path)
{
    ...
    TEST_ASSERT(nullptr != response.data);
    TEST_ASSERT(sizeof(RESPONSE) == response.size);
    TEST_EXPECT(0 == memcmp(response.data, RESPONSE, response.size));

    /* the server IV should be incremented. */
    TEST_EXPECT(EXPECTED_POST_IV == server_iv);
