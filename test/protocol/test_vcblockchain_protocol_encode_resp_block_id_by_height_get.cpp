/**
 * \file
 * test/protocol/test_vcblockchain_protocol_encode_resp_block_id_by_height_get.cpp
 *
 * Unit tests for encoding the block id by height get response.
 *
 ...
 */
TEST(test_vcblockchain_protocol_encode_resp_block_id_by_height_get, parameters)
{
...
TEST(test_vcblockchain_protocol_encode_resp_block_id_by_height_get, happy_path)
{
    const uint32_t EXPECTED_OFFSET = 26;
    const uint32_t EXPECTED_STATUS = 11;
    const vpr_uuid EXPECTED_BLOCK_ID = { .data = {
    ...
    EXPECT_EQ(htonl(PROTOCOL_REQ_ID_BLOCK_ID_BY_HEIGHT_GET), uarr[0]);
    ...
    EXPECT_EQ(0, memcmp(barr, &EXPECTED_BLOCK_ID, sizeof(EXPECTED_BLOCK_ID)));
