/**
 * \file
 * test/protocol/test_vcblockchain_protocol_encode_resp_latest_block_id_get.cpp
 *
 * Unit tests for encoding the latest block id get response.
 *
 ...
 */
TEST(test_vcblockchain_protocol_encode_resp_latest_block_id_get, parameters)
{
...
TEST(test_vcblockchain_protocol_encode_resp_latest_block_id_get, happy_path)
{
    const uint32_t EXPECTED_OFFSET = 26;
    const uint32_t EXPECTED_STATUS = 11;
    const vpr_uuid EXPECTED_BLOCK_ID = { .data = {
    ...
    EXPECT_EQ(htonl(PROTOCOL_REQ_ID_LATEST_BLOCK_ID_GET), uarr[0]);
