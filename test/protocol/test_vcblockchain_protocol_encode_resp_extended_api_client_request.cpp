...
TEST_SUITE(test_vcblockchain_protocol_encode_resp_extended_api_client_request);
...
TEST(parameter_checks)
{
    ...
    TEST_EXPECT(
        VCBLOCKCHAIN_ERROR_INVALID_ARG
            == vcblockchain_protocol_encode_resp_extended_api_client_request(
                    nullptr, &alloc_opts, EXPECTED_OFFSET, &client_id, &verb_id,
                    &client_enc_pubkey, &client_sign_pubkey, &request_body));
    TEST_EXPECT(
        VCBLOCKCHAIN_ERROR_INVALID_ARG
            == vcblockchain_protocol_encode_resp_extended_api_client_request(
                    &buffer, nullptr, EXPECTED_OFFSET, &client_id, &verb_id,
                    &client_enc_pubkey, &client_sign_pubkey, &request_body));
    TEST_EXPECT(
        VCBLOCKCHAIN_ERROR_INVALID_ARG
            == vcblockchain_protocol_encode_resp_extended_api_client_request(
    ...
    TEST_EXPECT(
        VCBLOCKCHAIN_ERROR_INVALID_ARG
            == vcblockchain_protocol_encode_resp_extended_api_client_request(
                    &buffer, &alloc_opts, EXPECTED_OFFSET, &client_id, nullptr,
                    &client_enc_pubkey, &client_sign_pubkey, &request_body));
    TEST_EXPECT(
        VCBLOCKCHAIN_ERROR_INVALID_ARG
            == vcblockchain_protocol_encode_resp_extended_api_client_request(
                    &buffer, &alloc_opts, EXPECTED_OFFSET, &client_id, &verb_id,
                    nullptr, &client_sign_pubkey, &request_body));
    TEST_EXPECT(
        VCBLOCKCHAIN_ERROR_INVALID_ARG
            == vcblockchain_protocol_encode_resp_extended_api_client_request(
                    &buffer, &alloc_opts, EXPECTED_OFFSET, &client_id, &verb_id,
                    &client_enc_pubkey, nullptr, &request_body));
    TEST_EXPECT(
        VCBLOCKCHAIN_ERROR_INVALID_ARG
            == vcblockchain_protocol_encode_resp_extended_api_client_request(
                    &buffer, &alloc_opts, EXPECTED_OFFSET, &client_id, &verb_id,
                    &client_enc_pubkey, &client_sign_pubkey, nullptr));
...
TEST(happy_path)
{
    ...
    size_t expected_buffer_size =
      ...
      + request_body.size;
    TEST_ASSERT(expected_buffer_size == buffer.size);
    ...
    TEST_EXPECT(
        PROTOCOL_REQ_ID_EXTENDED_API_CLIENTREQ == ntohl(net_request_id));
    ...
    TEST_EXPECT(client_enc_pubkey.size == ntohl(net_enc_key_size));
    ...
    TEST_EXPECT(client_sign_pubkey.size == ntohl(net_sign_key_size));
 
    /* the client id should be correct. */
    TEST_EXPECT(0 == memcmp(&client_id, barr, sizeof(client_id)));
    barr += sizeof(client_id);

    /* the verb id should be correct. */
    TEST_EXPECT(0 == memcmp(&verb_id, barr, sizeof(verb_id)));
    barr += sizeof(verb_id);

    /* the encryption pubkey should be correct. */
    TEST_EXPECT(
        0 == memcmp(client_enc_pubkey.data, barr, client_enc_pubkey.size));
    ...
    TEST_EXPECT(0 == memcmp(request_body.data, barr, request_body.size));
