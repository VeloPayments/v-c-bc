/**
 * \file
 * test/protocol/test_vcblockchain_protocol_encode_req_artifact_last_txn_id_get.cpp
 *
 * Unit tests for encoding the artifact last txn id get request.
 *
 ...
 */
TEST(
    test_vcblockchain_protocol_encode_req_artifact_last_txn_id_get,
    parameter_check)
{
 ...
 */
TEST(
    test_vcblockchain_protocol_encode_req_artifact_last_txn_id_get, happy_path)
{
    const uint32_t EXPECTED_OFFSET = 97;
    allocator_options_t alloc_opts;
    vccrypt_buffer_t buffer;
    ...
    EXPECT_EQ(htonl(PROTOCOL_REQ_ID_ARTIFACT_LAST_TXN_BY_ID_GET), u32arr[0]);
