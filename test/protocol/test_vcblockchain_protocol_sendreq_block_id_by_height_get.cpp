/**
 * \file test/protocol/test_vcblockchain_protocol_sendreq_block_id_by_height_get.cpp
 *
 * Unit tests for writing the block id by height get request to a server socket.
 *
...
TEST_SUITE(test_vcblockchain_protocol_block_id_by_height_get);
...
TEST(happy_path)
{
    ...
    TEST_EXPECT(PROTOCOL_REQ_ID_BLOCK_ID_BY_HEIGHT_GET == req.request_id);
    TEST_EXPECT(EXPECTED_OFFSET == req.offset);
    TEST_EXPECT(req.height == EXPECTED_HEIGHT);
