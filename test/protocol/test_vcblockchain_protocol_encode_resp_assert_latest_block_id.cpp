 ...
 */
TEST(test_vcblockchain_protocol_encode_resp_assert_latest_block_id, parameters)
{
...
TEST(test_vcblockchain_protocol_encode_resp_assert_latest_block_id, happy_path)
{
    const uint32_t EXPECTED_OFFSET = 26;
    const uint32_t EXPECTED_STATUS = 11;
    allocator_options_t alloc_opts;
    ...
    EXPECT_EQ(htonl(PROTOCOL_REQ_ID_ASSERT_LATEST_BLOCK_ID), uarr[0]);
