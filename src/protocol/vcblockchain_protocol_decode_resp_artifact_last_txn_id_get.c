/**
 * \file protocol/vcblockchain_protocol_decode_resp_artifact_last_txn_id_get.c
 *
 * \brief Decode a artifact last txn id get response into a struct.
 *
...
/* forward decls. */
static void dispose_protocol_resp_artifact_last_txn_id_get(void* disp);
...
static void dispose_protocol_resp_artifact_last_txn_id_get(void* disp)
{
    protocol_resp_artifact_last_txn_id_get* resp =
        (protocol_resp_artifact_last_txn_id_get*)disp;

    memset(resp, 0, sizeof(protocol_resp_artifact_last_txn_id_get));
}
