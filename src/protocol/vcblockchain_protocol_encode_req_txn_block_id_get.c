/**
 * \file protocol/vcblockchain_protocol_encode_req_txn_block_id_get.c
 *
 * \brief Encode a txn block id get request into a buffer.
 *
    ...
    uint32_t offset, const vpr_uuid* txn_id)
{
    ...
    u32arr[0] = htonl(PROTOCOL_REQ_ID_TRANSACTION_ID_GET_BLOCK_ID);
