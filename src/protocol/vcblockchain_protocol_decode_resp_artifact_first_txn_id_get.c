/**
 * \file protocol/vcblockchain_protocol_decode_resp_artifact_first_txn_id_get.c
 *
 * \brief Decode a artifact first txn id get response into a struct.
 *
...
/* forward decls. */
static void dispose_protocol_resp_artifact_first_txn_id_get(void* disp);
    ...
    size_t payload_size)
{
    ...
    resp->hdr.dispose = &dispose_protocol_resp_artifact_first_txn_id_get;
    const uint32_t* parr = (const uint32_t*)payload;
    resp->request_id = ntohl(parr[0]);
    resp->status = ntohl(parr[1]);
    resp->offset = ntohl(parr[2]);
    memcpy(&resp->first_txn_id, &(parr[3]), sizeof(resp->first_txn_id));
...
static void dispose_protocol_resp_artifact_first_txn_id_get(void* disp)
{
    protocol_resp_artifact_first_txn_id_get* resp =
        (protocol_resp_artifact_first_txn_id_get*)disp;

    memset(resp, 0, sizeof(protocol_resp_artifact_first_txn_id_get));
}
