/**
 * \file protocol/vcblockchain_protocol_decode_req_artifact_first_txn_id_get.c
 *
 * \brief Decode a artifact first txn id get request into a struct.
 *
...
/* forward decls. */
static void dispose_protocol_req_artifact_first_txn_id_get(void* disp);
    ...
    size_t payload_size)
{
    ...
    req->hdr.dispose = &dispose_protocol_req_artifact_first_txn_id_get;
    ...
    memcpy(&req->artifact_id, barr, 16);
...
static void dispose_protocol_req_artifact_first_txn_id_get(void* disp)
{
    protocol_req_artifact_first_txn_id_get* req =
        (protocol_req_artifact_first_txn_id_get*)disp;

    memset(req, 0, sizeof(protocol_req_artifact_first_txn_id_get));
}
