/**
 * \file protocol/vcblockchain_protocol_decode_resp_block_id_by_height_get.c
 *
 * \brief Decode a block id by height get response into a struct.
 *
...
/* forward decls. */
static void dispose_protocol_resp_block_id_by_height_get(void* disp);
    ...
    size_t payload_size)
{
    ...
    resp->hdr.dispose = &dispose_protocol_resp_block_id_by_height_get;
...
static void dispose_protocol_resp_block_id_by_height_get(void* disp)
{
    protocol_resp_block_id_by_height_get* resp =
        (protocol_resp_block_id_by_height_get*)disp;

    memset(resp, 0, sizeof(protocol_resp_block_id_by_height_get));
}
