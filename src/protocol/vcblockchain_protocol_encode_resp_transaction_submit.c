/**
 * \file protocol/vcblockchain_protocol_encode_resp_transaction_submit.c
 *
 * \brief Encode a transaction submit response into a buffer.
 *
    ...
    uint32_t offset, uint32_t status)
{
    ...
    size_t buffer_size = 3 * sizeof(uint32_t);
