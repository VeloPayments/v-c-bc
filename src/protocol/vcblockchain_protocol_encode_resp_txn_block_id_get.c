/**
 * \file protocol/vcblockchain_protocol_encode_resp_txn_block_id_get.c
 *
 * \brief Encode a txn block id get response into a buffer.
 *
    ...
    uint32_t offset, uint32_t status, const vpr_uuid* block_id)
{
    ...
    uarr[0] = htonl(PROTOCOL_REQ_ID_TRANSACTION_ID_GET_BLOCK_ID);
    uarr[1] = htonl(status);
    uarr[2] = htonl(offset);
