/**
 * \file protocol/vcblockchain_protocol_decode_resp_status_get.c
 *
 * \brief Decode a status get response into a struct.
 *
...
/* forward decls. */
static void dispose_protocol_resp_status_get(void* disp);
    ...
    protocol_resp_status_get* resp, const void* payload, size_t payload_size)
{
    ...
    resp->hdr.dispose = &dispose_protocol_resp_status_get;
...
static void dispose_protocol_resp_status_get(void* disp)
{
    protocol_resp_status_get* resp = (protocol_resp_status_get*)disp;

    memset(resp, 0, sizeof(protocol_resp_status_get));
}
