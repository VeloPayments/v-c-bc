/**
 * \file protocol/vcblockchain_protocol_encode_resp_block_id_by_height_get.c
 *
 * \brief Encode a block id by height get response into a buffer.
 *
    ...
    uint32_t offset, uint32_t status, const vpr_uuid* block_id)
{
    ...
    uarr[0] = htonl(PROTOCOL_REQ_ID_BLOCK_ID_BY_HEIGHT_GET);
    uarr[1] = htonl(status);
    uarr[2] = htonl(offset);
