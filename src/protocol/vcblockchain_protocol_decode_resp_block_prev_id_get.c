/**
 * \file protocol/vcblockchain_protocol_decode_resp_block_prev_id_get.c
 *
 * \brief Decode a block prev id get response into a struct.
 *
...
/* forward decls. */
static void dispose_protocol_resp_block_prev_id_get(void* disp);
...
static void dispose_protocol_resp_block_prev_id_get(void* disp)
{
    protocol_resp_block_prev_id_get* resp =
        (protocol_resp_block_prev_id_get*)disp;

    memset(resp, 0, sizeof(protocol_resp_block_prev_id_get));
}
