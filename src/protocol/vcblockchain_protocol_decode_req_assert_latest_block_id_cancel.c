/**
 * \file
 * protocol/vcblockchain_protocol_decode_req_assert_latest_block_id_cancel.c
 *
 * \brief Decode a latest block id assertion cancel request into a struct.
 *
...
/* forward decls. */
static void dispose_protocol_req_assert_latest_block_id_cancel(void* disp);
    ...
    size_t payload_size)
{
    ...
    req->hdr.dispose = &dispose_protocol_req_assert_latest_block_id_cancel;
...
static void dispose_protocol_req_assert_latest_block_id_cancel(void* disp)
{
    protocol_req_assert_latest_block_id_cancel* req =
        (protocol_req_assert_latest_block_id_cancel*)disp;

    memset(req, 0, sizeof(protocol_req_assert_latest_block_id_cancel));
}
