/**
 * \file
 * protocol/vcblockchain_protocol_decode_req_extended_api_enable.c
 *
 * \brief Decode an extended API enable request into a struct.
 *
...
/* forward decls. */
static void dispose_protocol_req_extended_api_enable(void* disp);
    ...
    size_t payload_size)
{
    ...
    req->hdr.dispose = &dispose_protocol_req_extended_api_enable;
...
static void dispose_protocol_req_extended_api_enable(void* disp)
{
    protocol_req_extended_api_enable* req =
        (protocol_req_extended_api_enable*)disp;

    memset(req, 0, sizeof(protocol_req_extended_api_enable));
}
