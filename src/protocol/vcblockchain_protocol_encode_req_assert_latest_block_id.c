/**
 * \file protocol/vcblockchain_protocol_encode_req_assert_latest_block_id.c
 *
    ...
    const vpr_uuid* latest_block_id)
{
    ...
    if (NULL == buffer || NULL == alloc_opts || NULL == latest_block_id)
    {
    ...
    /* copy the latest block id. */
    memcpy(u32arr + 2, latest_block_id, sizeof(*latest_block_id));
