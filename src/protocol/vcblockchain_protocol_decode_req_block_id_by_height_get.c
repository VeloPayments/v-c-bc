/**
 * \file protocol/vcblockchain_protocol_decode_req_block_id_by_height_get.c
 *
 * \brief Decode a block id by height get request into a struct.
 *
...
/* forward decls. */
static void dispose_protocol_req_block_id_by_height_get(void* disp);
    ...
    size_t payload_size)
{
    ...
    const size_t expected_payload_size = 2 * sizeof(uint32_t) + 8;
    ...
    req->hdr.dispose = &dispose_protocol_req_block_id_by_height_get;
    ...
    const uint64_t* b64 = (const uint64_t*)(u32arr + 2);
    req->height = ntohll(*b64);
...
static void dispose_protocol_req_block_id_by_height_get(void* disp)
{
    protocol_req_block_id_by_height_get* req =
        (protocol_req_block_id_by_height_get*)disp;

    memset(req, 0, sizeof(protocol_req_block_id_by_height_get));
}
