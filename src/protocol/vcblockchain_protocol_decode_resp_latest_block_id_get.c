/**
 * \file protocol/vcblockchain_protocol_decode_resp_latest_block_id_get.c
 *
 * \brief Decode a latest block id get response into a struct.
 *
...
/* forward decls. */
static void dispose_protocol_resp_latest_block_id_get(void* disp);
...
{
    protocol_resp_latest_block_id_get* resp =
        (protocol_resp_latest_block_id_get*)disp;

    memset(resp, 0, sizeof(protocol_resp_latest_block_id_get));
}
