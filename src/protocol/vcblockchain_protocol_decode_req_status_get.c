/**
 * \file protocol/vcblockchain_protocol_decode_req_status_get.c
 *
 * \brief Decode a status get request into a struct.
 *
...
/* forward decls. */
static void dispose_protocol_req_status_get(void* disp);
    ...
    protocol_req_status_get* req, const void* payload, size_t payload_size)
{
    ...
    req->hdr.dispose = &dispose_protocol_req_status_get;
...
static void dispose_protocol_req_status_get(void* disp)
{
    protocol_req_status_get* req = (protocol_req_status_get*)disp;

    memset(req, 0, sizeof(protocol_req_status_get));
}
