/**
 * \file protocol/vcblockchain_protocol_sendreq_block_id_by_height_get.c
 *
 * \brief Send a block id by height get request to the server.
 *
    ...
    vccrypt_buffer_t* shared_secret, uint32_t offset, uint64_t height)
{
    ...
    retval =
        vcblockchain_protocol_encode_req_block_id_by_height_get(
            &buffer, suite->alloc_opts, offset, height);
