/**
 * \file protocol/vcblockchain_protocol_encode_resp_txn_next_id_get.c
 *
 * \brief Encode a txn next id get response into a buffer.
 *
    ...
    uint32_t offset, uint32_t status, const vpr_uuid* next_txn_id)
{
    ...
    if (NULL == buffer || NULL == alloc_opts || NULL == next_txn_id)
    {
    ...
    uarr[0] = htonl(PROTOCOL_REQ_ID_TRANSACTION_ID_GET_NEXT);
    uarr[1] = htonl(status);
    uarr[2] = htonl(offset);
    memcpy(&(uarr[3]), next_txn_id, sizeof(vpr_uuid));
