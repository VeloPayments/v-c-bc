/**
 * \file protocol/vcblockchain_protocol_decode_resp_txn_prev_id_get.c
 *
 * \brief Decode a txn prev id get response into a struct.
 *
...
/* forward decls. */
static void dispose_protocol_resp_txn_prev_id_get(void* disp);
...
static void dispose_protocol_resp_txn_prev_id_get(void* disp)
{
    protocol_resp_txn_prev_id_get* resp =
        (protocol_resp_txn_prev_id_get*)disp;

    memset(resp, 0, sizeof(protocol_resp_txn_prev_id_get));
}
