/**
 * \file protocol/vcblockchain_protocol_encode_resp_connection_close.c
 *
 * \brief Encode a connection_close get response into a buffer.
 *
    ...
    uint32_t offset, uint32_t status)
{
    ...
    uarr[0] = htonl(PROTOCOL_REQ_ID_CLOSE);
    uarr[1] = htonl(status);
    uarr[2] = htonl(offset);
