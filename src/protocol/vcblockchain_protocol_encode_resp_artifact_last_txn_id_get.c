/**
 * \file protocol/vcblockchain_protocol_encode_resp_artifact_last_txn_id_get.c
 *
 * \brief Encode an artifact last txn id get response into a buffer.
 *
    ...
    uint32_t offset, uint32_t status, const vpr_uuid* last_txn_id)
{
    ...
    if (NULL == buffer || NULL == alloc_opts || NULL == last_txn_id)
    {
    ...
    uarr[0] = htonl(PROTOCOL_REQ_ID_ARTIFACT_LAST_TXN_BY_ID_GET);
    uarr[1] = htonl(status);
    uarr[2] = htonl(offset);
    memcpy(&(uarr[3]), last_txn_id, sizeof(vpr_uuid));
