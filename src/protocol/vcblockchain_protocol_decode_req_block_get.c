/**
 * \file protocol/vcblockchain_protocol_decode_req_block_get.c
 *
 * \brief Decode a block get request into a struct.
 *
...
/* forward decls. */
static void dispose_protocol_req_block_get(void* disp);
    ...
    protocol_req_block_get* req, const void* payload, size_t payload_size)
{
    ...
    req->hdr.dispose = &dispose_protocol_req_block_get;
...
static void dispose_protocol_req_block_get(void* disp)
{
    protocol_req_block_get* req =
        (protocol_req_block_get*)disp;

    memset(req, 0, sizeof(protocol_req_block_get));
}
